...
import { MovimientoResponse } from '../../types/movimiento';
...
const MovimientosListPage: React.FC = () => {
    ...
    const columns = useMemo(() => {
        ...
        return [
            ...
            { Header: 'Fecha', accessor: 'fecha_movimiento', Cell: ({ row }: TableCellProps) => new Date(row.original.fecha_movimiento).toLocaleString() },
        ];
    }, [movimientos]);
    ...
    return (
        <div className={`${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'} min-h-screen p-4`}>
            ...
            <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} p-4 rounded-lg shadow-sm flex flex-wrap items-center gap-4 mb-4`}>
                ...
                <div>
                     <Select id="tipoMovimientoFilter" value={tipoMovimientoFilter} onChange={(e) => handleFilterValueChange(setTipoMovimientoFilter)(e.target.value)} options={[
