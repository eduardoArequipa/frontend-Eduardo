...
const ProductosListPage: React.FC = () => {
     ...
     useEffect(() => {
          const loadFilterData = async () => {
               ...
               try {
                    ...
                    setAvailableCategoriasFilter(categoriasData);
    ...
    const handleDelete = async (id: number, nombreProducto: string) => {
        if (window.confirm(`¿Estás seguro de desactivar el producto "${nombreProducto}"?`)) {
            try {
                ...
                fetchProductos(); // Recargar para reflejar el cambio
                alert(`Producto "${nombreProducto}" desactivado con éxito!`);
            } catch (err: any) {
                 console.error(`Error deactivating producto ${id}:`, err.response?.data || err);
     ...
     const handleActivate = async (id: number, nombreProducto: string) => {
         if (window.confirm(`¿Estás seguro de activar el producto "${nombreProducto}"?`)) {
             try {
                 ...
                 console.error(`Error activating producto ${id}:`, err.response?.data || err);
        ...
        return [
            {
                ...
                Cell: ({ row }: TableCellProps) => {
                    const imagenRuta = row.original.imagen_ruta;
                    const fullImageUrl = (imagenRuta && imagenRuta.startsWith('/'))
                        ? `${BACKEND_BASE_URL}${imagenRuta}`
                        : '/src/assets/default-product-image.png';
                    return <img src={fullImageUrl} alt={`Imagen de ${row.original.nombre}`} className="h-10 w-10 object-cover rounded-md" onError={(e) => { e.currentTarget.src = '/src/assets/image-error.png'; }} />;
                },
            },
            { Header: 'Código', accessor: 'codigo' },
            { Header: 'Nombre', accessor: 'nombre' },
            { Header: 'Precio Compra', accessor: 'precio_compra', Cell: ({ row }: TableCellProps) => <span className="text-blue-700 font-semibold">{Number(row.original.precio_compra).toFixed(2)} Bs.</span> },
            { Header: 'Precio Venta', accessor: 'precio_venta', Cell: ({ row }: TableCellProps) => <span className="text-green-700 font-semibold">{Number(row.original.precio_venta).toFixed(2)} Bs.</span> },
            {
                Header: 'Stock',
                accessor: 'stock',
                Cell: ({ row }: TableCellProps) => {
                    ...
                    let textColorClass = stock <= stockMinimo ? 'text-red-900' : stock > stockMinimo * 2 ? 'text-green-900' : 'text-orange-900';
                    let bgColorClass = stock <= stockMinimo ? 'bg-red-200' : stock > stockMinimo * 2 ? 'bg-green-200' : 'bg-orange-200';
            ...
            },
            {
                ...
                accessor: 'estado',
                Cell: ({ row }: TableCellProps) => (
                    <span className={`relative inline-block px-3 py-1 font-semibold leading-tight ${row.original.estado === EstadoEnum.Activo ? 'text-green-900' : 'text-red-900'}`}>
                        <span aria-hidden className={`absolute inset-0 opacity-50 rounded-full ${row.original.estado === EstadoEnum.Activo ? 'bg-green-200' : 'bg-red-200'}`}></span>
            ...
            {
                Header: 'Acciones',
                accessor: 'acciones', // Add accessor property as required by Column type
                Cell: ({ row }: TableCellProps) => {
                    ...
                    const productActions: ActionConfig[] = [
                        ...
                        },
                        {
                            ...
                            isVisible: producto.estado === EstadoEnum.Activo,
                            buttonVariant: 'menuItem',
                            colorClass: 'text-red-700'
                        },
                        {
                            label: 'Activar',
                            onClick: () => handleActivate(producto.producto_id, producto.nombre),
                            isVisible: producto.estado === EstadoEnum.Inactivo,
                            buttonVariant: 'menuItem',
    ...
    if ((loading || loadingFilterData) && productos.length === 0) {
        return <div className="flex justify-center items-center min-h-[calc(100vh-200px)]"><LoadingSpinner /> Cargando productos...</div>;
    }
        ...
        <div>
            ...
            <div className="mb-4 bg-white p-4 rounded-lg shadow-sm flex flex-wrap items-center gap-4">
                ...
                <div>
                 ...
                 <div className="flex-grow md:flex-none flex justify-end">
                     <Button onClick={handleOpenAddModal} className="bg-green-600 hover:bg-green-700 text-white">Crear Nuevo Producto</Button>
                 </div>
            </div>

            {loading && productos.length > 0 && <div className="absolute inset-0 flex justify-center items-center bg-white bg-opacity-75 z-10"><LoadingSpinner /></div>}
            ...
            {productos.length === 0 && !loading && !error ? (
             ...
             <div className="mt-4 flex justify-center items-center space-x-4">
                 ...
                 <Button onClick={() => setCurrentPage(prev => prev + 1)} disabled={loading}>Siguiente</Button>
             </div>

            {/* Modal para CREAR Nuevo Producto */}
            <Modal isOpen={isAddModalOpen} onClose={handleCloseAddModal} title="Crear Nuevo Producto" widthClass="max-w-3xl" showCancelButton={false} showConfirmButton={false}>
            ...
            {editingProduct && (
                <Modal isOpen={isEditModalOpen} onClose={handleCloseEditModal} title={`Editar Producto: ${editingProduct.nombre}`} widthClass="max-w-3xl" showCancelButton={false} showConfirmButton={false}>
